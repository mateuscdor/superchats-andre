...
var recordToLines = function recordToLines(record, options) {
  var src = record.src,
      ...
      _record$shortId = record.shortId,
      shortId = _record$shortId === undefined ? ' ' : _record$shortId,
      obj = record.obj,
  ...
  if (fStory) {
    ...
    switch (record.action) {
      ...
      case 'CLOSED':
        ...
        break;
      default:
        prefix = fRoot ? '\u251C\u2500\u2500\u2500 ' : '\u2502' + shortId + '\u251C\u2500 ';
        break;
    }
    msgStr = _chalk2.default.bold(record.title);
    actionStr = ' [' + _chalk2.default.dim(shortId) + '\xB7' + _chalk2.default.bold(record.action) + ']';
  } else {
    // parents = [storyId];
  ...
  prefix = _chalk2.default.dim(prefix);
  ...
  var finalMsg = timeStr + ' ' + srcStr + ' ' + levelStr + prefix + msgStr + actionStr + objStr;
  ...
  if (fHasObj && objExpanded && filters.passesFilter(src, objLevel)) {
    ...
    lines.forEach(function (line) {
      var text = emptyTimeStr + ' ' + srcStr + ' ' + levelStr2 + prefix + line;
